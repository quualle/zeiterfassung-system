-- Activity Log Schema for Supabase (Fixed version)
...
-- For now, allow all access (since we don't have auth system)
-- In production, you would check for admin role
CREATE POLICY "Allow all access to activities" ON activities
    FOR ALL USING (true);

CREATE POLICY "Allow all access to sync_status" ON sync_status
    FOR ALL USING (true);
